...
public interface HivEnrollmentRepository extends JpaRepository<HivEnrollment, Long> {
    ...
    @Query(value = "SELECT p.id AS id, p.first_name as firstName, p.surname AS surname, p.other_name AS otherName, \n" +
            ...
            "INNER JOIN  hiv_art_clinical ca ON  p.uuid = ca.person_uuid\n" +
            "INNER JOIN  base_application_codeset  pc on pc.id = e.status_at_registration_id\n" +
            "WHERE p.archived=0 AND p.facility_id=?1\n" +
            "AND ca.is_commencement = true\n" +
            ...
            nativeQuery = true)
    Page<PatientProjection> getPatientByFacilityId(Long facilityId, Pageable page);
}
