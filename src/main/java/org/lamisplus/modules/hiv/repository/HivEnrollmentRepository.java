...
public class HivEnrollmentRepository {
    ...
    public List<HivEnrollment> findAll() {
        return jdbcTemplate.query("SELECT id, archived, breastfeeding, created_by, " +
                        "date_confirmed_hiv, date_created, date_enrolled_pmtct, " +
                        "date_modified, date_of_registration, date_started, " +
                        "enrollment_setting_id, entry_point_id, extra, " +
                        "modified_by, person_id, pregnant, " +
                        "send_message, source_of_referrer, status_at_registration_id, " +
                        "target_group_id, tb_status_id, time_hiv_diagnosis, unique_id, uuid FROM hiv_enrollment",
    ...
    public HivEnrollmentDTO save(HivEnrollment hivEnrollment) {
        ...
        if (hivEnrollment.getId() == null || hivEnrollment.getId() == 0) {
            jdbcTemplate.update("INSERT INTO hiv_enrollment(id, archived, breastfeeding, " +
                    ...
                    extra, hivEnrollment.getModifiedBy(), hivEnrollment.getPersonId(),
                    hivEnrollment.getPregnant(), hivEnrollment.getSendMessage(), hivEnrollment.getSourceOfReferrer(),
                    hivEnrollment.getStatusAtRegistrationId(), hivEnrollment.getTargetGroupId(), hivEnrollment.getTbStatusId(),
                    hivEnrollment.getTimeHivDiagnosis(), hivEnrollment.getUniqueId(), hivEnrollment.getUuid());

        } else {
            jdbcTemplate.update("UPDATE hiv_enrollment SET archived=?, breastfeeding=?, date_confirmed_hiv=?, " +
                            "date_enrolled_pmtct=?, date_modified=?, date_of_registration=?, " +
                            "date_started=?, enrollment_setting_id=?, entry_point_id=?, " +
                            "extra=(to_json(?::jsonb)), modified_by=?, person_id=?, " +
                            "pregnant=?, send_message=?, source_of_referrer=?, " +
                            "status_at_registration_id=?, target_group_id=?, tb_status_id=?, " +
                            "time_hiv_diagnosis=?, unique_id=?)" +
                            " WHERE id=?",
                    hivEnrollment.getArchived(), hivEnrollment.getBreastfeeding(), hivEnrollment.getDateConfirmedHiv(),
                    hivEnrollment.getDateEnrolledPMTCT(), hivEnrollment.getDateModified(), hivEnrollment.getDateOfRegistration(),
                    hivEnrollment.getDateStarted(), hivEnrollment.getEnrollmentSettingId(), hivEnrollment.getEntryPointId(),
                    extra, hivEnrollment.getModifiedBy(), hivEnrollment.getPersonId(),
                    hivEnrollment.getPregnant(), hivEnrollment.getSendMessage(), hivEnrollment.getSourceOfReferrer(),
                    hivEnrollment.getStatusAtRegistrationId(), hivEnrollment.getTargetGroupId(), hivEnrollment.getTbStatusId(),
                    hivEnrollment.getTimeHivDiagnosis(), hivEnrollment.getUniqueId(), hivEnrollment.getId());
        }
    ...
    public Optional<HivEnrollmentDTO> findById(Long id) {
        return Optional.ofNullable(jdbcTemplate.queryForObject("SELECT * FROM hiv_enrollment WHERE id = ?", new Object[]{id}, new HivEnrollmentRowMapper()));
    }

    public Optional<HivEnrollmentDTO> findByUUID(String uuid) {
        return Optional.ofNullable(jdbcTemplate.queryForObject("SELECT * FROM hiv_enrollment WHERE uuid = ?", new Object[]{uuid}, new HivEnrollmentRowMapper()));
    }
}
