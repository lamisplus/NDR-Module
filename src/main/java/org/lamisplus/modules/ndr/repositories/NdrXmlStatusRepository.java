...
public interface NdrXmlStatusRepository extends JpaRepository<NdrXmlStatus, Integer> {
	...
	@Query(value = "SELECT p.id,p.uuid as personUuid, p.facility_id as facilityId, p.archived \\:\\:BOOLEAN as archived, p.uuid,p.hospital_number as hospitalNumber, \n" +
			...
			"\t\t\t\t INNER JOIN base_organisation_unit_identifier boui ON boui.organisation_unit_id=facility_id\n" +
			"\t\t\t\t INNER JOIN hiv_enrollment h ON h.person_uuid = p.uuid\n" +
			"\t\t\t\t WHERE h.archived=0 AND p.uuid IN :patientUuids",nativeQuery = true)
	List<PatientDemographics> getPatientDemographicsByUUIDIn(@Param("patientUuids") List<String> patientUuids);
	...
	@Query(value = "SELECT c.id as clinicId,\n" +
			...
			"from public.hiv_art_clinical c\n" +
			"inner join triage_vital_sign  v on v.uuid = c.vital_sign_uuid\n" +
			"where is_commencement is false\n" +
			"and c.archived = 0 and c.person_uuid IN :personUuids", nativeQuery = true)
	List<ARTClinicalInfo> getClinicalInfoByUUIDIn(@Param("personUuids") List<String> personUuids);
