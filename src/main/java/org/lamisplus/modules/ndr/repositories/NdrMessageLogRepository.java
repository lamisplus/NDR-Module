...
public interface NdrMessageLogRepository extends JpaRepository<NdrMessageLog, Integer> {
    ...
    @Query(value="select * from (SELECT\n" +
            ...
            "            COALESCE(ndrFuncStatCodestatus.code, ndrClinicStage.code) AS functionalStatusStartART,\n" +
            "            CASE WHEN hpt.reason_for_discountinuation = 'Death' THEN hpt.cause_of_death ELSE NULL END AS causeOfDeath, \n" +
            "            ROW_NUMBER() over (PARTITION BY p.uuid order by hac.visit_date asc) as rn \n"+
            "               FROM\n" +
            "                    patient_person p\n" +
            "                       INNER JOIN base_organisation_unit facility ON facility.id = facility_id\n" +
            "                        INNER JOIN base_organisation_unit facility_lga ON facility_lga.id = facility.parent_organisation_unit_id\n" +
            ...
            "               WHERE h.archived = 0\n" +
            "               AND h.facility_id = ?1\n" +
            "               AND hac.is_commencement = TRUE) as dt where rn=1\n" ,
            nativeQuery = true)
    HashSet<PatientDemographicDTO> getAllPatientDemographics(Long facilityId);
    ...
    @Query(value="select * from get_patient_labs(?1, ?2, ?3)", nativeQuery = true)
...
//    Optional<PatientPharmacyEncounterDTO> getPatientLastPharmacyEncounter(String identifier, Long facilityId);
  ...
  Optional<PatientDemographicDTO> getHtsPatientDemographics(long facilityId,  String clientCode, LocalDateTime lastModified);
    ...
    @Query(value = "select * from generate_hts_demography_ndr(?1, ?2)", nativeQuery = true)
    HashSet<PatientDemographicDTO> getAllHtsPatientDemographics(long facilityId, LocalDateTime lastModified);
...
//          "AND hc.date_modified > ?3 AND hc.archived = 0",nativeQuery = true)
  ...
  List<HtsReportDto> getHstReportByClientCodeAndLastModified(Long facilityId, String clientCode, LocalDateTime lastModified);
    ...
    @Query(value = "select * from generate_hts_ndr(?1, ?2)", nativeQuery = true)
            ArrayList<HtsReportDto> getAllHTSReport(Long facilityId, LocalDateTime lastModified);
